...
def classification_training_pipeline(base_path, model_type, dataset_name, device, epochs=100, lr=0.001, model_load_path=""):
    ...
    # load model
    network = get_classification_model(model_type, device, input_channels, load_from_path=model_load_path)

    loss_plot_path = base_path + "/images/"+model_type+"_"+dataset_name
    # train model
    network = train_classification_model(network, train_loader, val_loader, device, model_save_path, loss_plot_path, epochs=epochs, lr=lr)
